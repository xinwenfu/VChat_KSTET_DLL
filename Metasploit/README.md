...
Then we can select the `KSTET_DLL` module with the [`use`](https://www.offsec.com/metasploit-unleashed/msfconsole-commands/#use) command followed by the index of the module we want to use. In my case this was `4`.
...
1. Show options
   ...
   * The *Module Options* are those specific to our module. The `RETOFFSET_KSTET` is a custom option for telling the exploit the offset to the *return* value on the stack for the KSTET command. The `SHORT_JUMP` option contains the shellcode used to preform a short jump after the `JMP ESP` instruction is executed. The other options like `RHOST` and `RPORT` are use to control the remote connections our exploit will make and are from the ` Msf::Exploit::Remote` class we inherit.
   * The `Payload` options are used to control the behavior of the payload. With `EXITFUNC` telling the payload generator how the exploit will be ran (Thread vs Process), then `LHOST` and `LPORT` tell the payload how to connect back to up. The `LHOST` and `LPORT` are set automatically by Metasploit in most cases, if you have multiple interfaces it may not set the right one. As we will be using the `custom/generic`

2. Configure Payload, this also use the keyword `set` followed by the word `payload` and the one we would like to use in the exploit. In this case as we are using custom shellcode we will need to use the `generic/custom` payload.
 
    <img src="Images/MSFPayload.png">

3. Now we can set the shellcode, or shellcode file. This would be a hex string that will later be encoded.
...
> [!IMPORTANT]
> You will need to setup a netcat listener on the kali machine.
