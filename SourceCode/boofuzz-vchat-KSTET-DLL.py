...
def main():
    ...
    # Define the fuzzing structure for the "KSTET" command
    s_initialize("KSTET")  # Initialize a fuzzing block named "KSTET"
    s_string('KSTET', fuzzable=False, name='KSTET-Command')  # Send the command "KSTET" (not fuzzable)
    s_delim(' ', fuzzable=False, name='KSTET-Space')  # Send a space after "KSTET" (not fuzzable)
    s_string('A', name='KSTET-STRING')  # Fuzz this part by sending different "A" variations
    s_static('\r\n', name='KSTET-CRLF')  # Append a carriage return + newline to mimic real commands

    session.pre_send = receive_banner  # Assign `receive_banner` as a pre-send callback to read any initial response

    session.connect(s_get("KSTET"))  # Connect the fuzzing session to the "KSTET" structure defined above
